  ...
  <div class="visitor-edit-page">
    <div class="edit-header">
      <nav class="breadcrumb">
        <router-link to="/visitors" class="breadcrumb-item">访问器</router-link>
        <span class="breadcrumb-separator">›</span>
        <span class="breadcrumb-current">{{ isEditing ? '编辑访问器' : '新建访问器' }}</span>
      </nav>
...
const formRules: FormRules = {
  name: [
    ...
    { min: 1, max: 50, message: '长度应在1到50之间', trigger: 'blur' },
  ],
...
const loadVisitor = async () => {
  ...
  try {
    ...
    ElMessage.error('加载访问器失败：' + err.message)
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      ElMessage.success('访问器已更新')
    } else {
      await visitorStore.createVisitor(data)
      ElMessage.success('访问器已创建')
    }
