  ...
  <div class="visitors-page">
    ...
    <div v-loading="visitorStore.loading">
      <div v-if="!visitorStore.storeEnabled" class="store-disabled">
        <p>Store 未启用。请在 frpc 配置中添加以下内容：</p>
...
path = "./frpc_store.json"</pre>
      ...
      <template v-else>
        ...
        <div v-else class="empty-state">
          ...
          <p class="empty-hint">点击「新建访问器」创建一个。</p>
        </div>
      </template>
    </div>

    <ConfirmDialog v-model="deleteDialog.visible" title="删除访问器"
...
const handleDelete = (name: string) => {
  ...
  deleteDialog.message = `确定要删除访问器「${name}」吗？此操作无法撤销。`
